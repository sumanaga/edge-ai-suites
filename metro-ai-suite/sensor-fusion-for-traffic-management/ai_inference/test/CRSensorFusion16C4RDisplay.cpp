...
void workload(const std::string& host, const std::string& port,
              ...
              MessageBuffer_t* pb, unsigned thread_id, unsigned stream_num,
              unsigned pipeline_repeats, bool warmup_flag) {
    ...
    std::thread reader([&]() {
      ...
      while (stream->Read(&reply)) {
        ...
        try {
          ...
          if (pb->write_pos == MAX_MESSAGE_BUFFER_SIZE) pb->write_pos = 0;
...
void parseReply(cv::Mat& frame, Plot& pl, float resize_rate, std::string imsg,
  ...
  if (jsonTree.count("latency") > 0) {
    ...
    if (jsonTree.count("latency2") > 0) {
      latency = jsonTree.get<double>("latency2");
      g_latency.push_back(latency);
    }
    if (jsonTree.count("latency3") > 0) {
      latency = jsonTree.get<double>("latency3");
      g_latency.push_back(latency);
    }
    if (jsonTree.count("latency4") > 0) {
      latency = jsonTree.get<double>("latency4");
      g_latency.push_back(latency);
    }
    ...
    if (jsonTree.count("inference_latency2") > 0) {
      latency = jsonTree.get<double>("inference_latency2");
      g_inference_latency.push_back(latency);
    }
    if (jsonTree.count("inference_latency3") > 0) {
      latency = jsonTree.get<double>("inference_latency3");
      g_inference_latency.push_back(latency);
    }
    if (jsonTree.count("inference_latency4") > 0) {
      latency = jsonTree.get<double>("inference_latency4");
      g_inference_latency.push_back(latency);
    }
    ...
    if (jsonTree.count("video_latency2") > 0) {
      latency = jsonTree.get<double>("video_latency2");
      g_video_latency.push_back(latency);
    }
    if (jsonTree.count("video_latency3") > 0) {
      latency = jsonTree.get<double>("video_latency3");
      g_video_latency.push_back(latency);
    }
    if (jsonTree.count("video_latency4") > 0) {
      latency = jsonTree.get<double>("video_latency4");
      g_video_latency.push_back(latency);
    }
    ...
    for (auto roi_i = roi_info.begin(); roi_i != roi_info.end(); ++roi_i) {
      ...
      int64_t sensor_source = roi_item.get<int64_t>("sensor_source");
      ...
      if ((x == 0 && y == 0 && w == 0 && h == 0) || (-1 == sensor_source)) {
        ...
        cv::Point start_point(
            x_resize + raw_image_width * ((thread_id * 4 + sensor_source) / 4),
            y_resize +
                raw_image_height * ((thread_id * 4 + sensor_source) % 4));
        cv::Point end_point(
            x_resize + w_resize +
                raw_image_width * ((thread_id * 4 + sensor_source) / 4),
            y_resize + h_resize +
                raw_image_height * ((thread_id * 4 + sensor_source) % 4));
...
void renderOSDImage(cv::Mat raw_image, unsigned thread_id,
  ...
  cv::resize(
      raw_image, raw_image,
      Size(display_params.frameSize.width, display_params.frameSize.height));
  cv::resize(plot, plot, Size(plot.cols * 2, plot.rows * 2));
  // cv::hconcat(raw_image, plot, osd_image);

  std::lock_guard<std::mutex> lg(g_window_mutex);
  for (int i = thread_id * 4; i < (thread_id + 1) * 4; i++) {
    raw_image.copyTo(windowImage(cv::Rect(display_params.points[i].x,
                                          display_params.points[i].y,
                                          raw_image.cols, raw_image.rows)));
  }

  float resize_rate = static_cast<float>(frame_height / (plot.rows / 4));
...
int main(int argc, char** argv) {
  try {
    ...
    if (argc < 8 || argc > 13) {
      std::cerr
          << "Usage: CRSensorFusion16C4RDisplay <host> <port> <json_file> "
             "<total_stream_num> <repeats> <data_path> <display_type> "
             "[<save_flag: 0 | 1>] [<pipeline_repeats>] [<cross_stream_num>] "
             "[<warmup_flag: 0 | 1>] [<logo_flag: 0 | 1>]\n"
          << "Example:\n"
          << "    ./CRSensorFusion16C4RDisplay 127.0.0.1 50052 "
             "../../ai_inference/test/configs/raddet/16C4R/"
             "localFusionPipeline.json 4 1 "
    ...
    if (argc == 11) {
      saveFlag = bool(atoi(argv[8]));
      pipeline_repeats = atoi(argv[9]);
      crossStreamNum = atoi(argv[10]);
    }
    if (argc == 12) {
      saveFlag = bool(atoi(argv[8]));
      pipeline_repeats = atoi(argv[9]);
      crossStreamNum = atoi(argv[10]);
      warmupFlag = bool(atoi(argv[11]));
    }
    if (argc == 13) {
      saveFlag = bool(atoi(argv[8]));
      pipeline_repeats = atoi(argv[9]);
    ...
    if (totalStreamNum / crossStreamNum * crossStreamNum != totalStreamNum) {
      std::cerr
          << "total-stream-number should be divisible by cross-stream-number!"
          << std::endl;
      return EXIT_FAILURE;
    }
    ...
    FusionType_t fusionType = FusionType_t::FUSION_16C4R;
    DisplayParams_t params =
        prepareDisplayParams(fusionType, 16, WINDOWNAME, pl);
    ...
    for (unsigned i = 0; i < threadNum; ++i) {
      ...
      std::thread t(workload, std::ref(host), std::ref(port),
                    ...
                    &msg_buffer[i], i, crossStreamNum, pipeline_repeats,
                    warmupFlag);
    ...
    for (unsigned j = 0; j < threadNum; ++j) {
      ...
      std::thread d(OSDload, std::ref(pipeline_repeatinputs), threadNum, j,
                    windowImage, params, &msg_buffer[j], std::ref(pl),
                    mediaType, displayType, showMetrics, saveFlag, logoFlag);
    ...
    std::cout << "inference average latency " << inference_latency_ave
              << std::endl;
    std::cout << "For each repeat: " << threadNum
