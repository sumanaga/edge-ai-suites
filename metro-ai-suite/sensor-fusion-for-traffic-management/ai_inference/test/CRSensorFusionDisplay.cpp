...
void parseReply(cv::Mat& frame, Plot& pl, float resize_rate, std::string imsg,
        ...
        if ("radar" != display_type) {
          ...
          if (x == 0 && y == 0 && w == 0 && h == 0) {
            ...
            sprintf(objectStr, "[%s]: %0.2f", roi_class_name.c_str(),
                    roi_score);
            cv::putText(frame, objectStr, start_point2,
                        cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar{0, 127, 255},
                        1, cv::LINE_8);
            cv::rectangle(frame, start_point, end_point, cv::Scalar{0, 255, 0},
                          1, cv::LINE_8);
          }
  ...
  cv::putText(frame, fpsStr, cv::Point(frame.cols - textSize.width, offset),
              font, fontScale, {0, 255, 0}, thickness, cv::LINE_8);
  ...
  cv::putText(frame, frameCntStr,
              cv::Point(frame.cols - textSize.width, offset), font, fontScale,
              {0, 255, 0}, thickness, cv::LINE_8);
  ...
  try {
    ...
    if (argc < 8 || argc > 14) {
      std::cerr << "Usage: CRSensorFusionDisplay <host> <port> <json_file> "
                   ...
                   "[<logo_flag: 0 | 1>]\n"
                << "Example:\n"
                << "    ./CRSensorFusionDisplay 127.0.0.1 50052 "
                   "../../ai_inference/test/configs/raddet/"
                   "localFusionPipeline.json 1 1 "
